...
class GridData():
	...
	def vector_ends(self, mcolats, mlons, kvecs, los_vs, dtime=0, mlt=True):
		
		"""
		Calculates endpoints of los vectors based on kvector
		
		Parameters
		----------
		
		mcolats: float array
			magnetic colatitude of data
			
		mlons: float array
			magnetic longitude of data (in degrees)
			
		kvecs: float array
			kvector of data (in degrees)
			
		los_vs: float array
			line of sight velocities (in m/s)
			
		dtime: dtime object, optional
			time of measurement, used to convert from magnetic longitude to 
			magnetic local time of format 
		...
		dtheta = vec_lon - np.deg2rad(mlons)
		
		return dr, np.rad2deg(dtheta)
	
	def vector_plot(self, time, mlt=True):
		
		"""
		Creates a vector plot of data
		
		Parameters
		----------
		
		time: string
			day to plot data for (in format "YYYY/MM/DD HH:mm/ss")
		
		mlt: bool
			if true, sets to convert into magnetic local time
		"""
		
		#find where the data is for our date
		time_index = np.where(self.times == time)[0]
		...
		if not self.mod:
			...
			cNorm = mpl.colors.Normalize(vmin=0, vmax=600)
		cm = mpl.cm.jet
		...
		#get our mcolats, mlons, kvecs and los_vs
		mcolats = self.mcolats[time_index]
		mlons = self.mlons[time_index]
		kvecs = self.kvecs[time_index]
		los_vs = self.los_vs[time_index]
		dtime = self.dtimes[time_index][0]
		...
		#get dr, dtheta
		dr, dtheta = self.vector_ends(mcolats, mlons, kvecs, los_vs, dtime, mlt)
		self.dr = dr
		
		#plot stations
		for station in self.station_metadata:
			#get station object
			radar = self.station_metadata[station]
			#get aacgm coordinates
			radar.set_aacgm(dtime)
			#get magnetic local time
			if mlt:
				radar.mlon = radar.get_mlt(dtime)*15
			#BKS.mlon = tools.aacgm_to_mlt(BKS.mlon, dtime)[0]*15
			ax.scatter(np.deg2rad(radar.mlon), radar.mcolat, 5)
			ax.annotate(station, [np.deg2rad(radar.mlon), radar.mcolat])
